...
import star.api.chart.manager.RedisLimiterManager;
import star.api.chart.mapper.ChartMapper;
import star.api.chart.mq.BiMessageProducer;
import star.api.chart.service.ChartService;
import star.api.chart.utils.ExcelUtils;
...
public class ChartServiceImpl extends ServiceImpl<ChartMapper, Chart>
    ...
    public Boolean deleteChart(@RequestBody DeleteRequest deleteRequest, HttpServletRequest request) {
        ...
        // 仅本人或管理员可删除
        if (!oldChart.getUserId().equals(user.getId()) && !innerUserService.isAdmin(token)) {
    ...
    public Boolean editChart(@RequestBody ChartEditRequest chartEditRequest, HttpServletRequest request) {
        ...
        ThrowUtils.throwIf(oldChart == null, ErrorCode.NOT_FOUND_ERROR);
        // 仅本人或管理员可编辑
        ThrowUtils.throwIf(!oldChart.getUserId().equals(loginUser.getId()) && !innerUserService.isAdmin(token), ErrorCode.FORBIDDEN_ERROR);
    ...
    public AiResultVO genChartByAi(@RequestPart("file") MultipartFile multipartFile, GenChartByAiRequest genChartByAiRequest, HttpServletRequest request) {
        ...
        String token = request.getHeader("Authorization");
        User loginUser = innerUserService.getLoginUser(token);
        redisLimiterManager.doRateLimit("genChartByAi_" + innerUserService.getLoginUser(token).getId());
